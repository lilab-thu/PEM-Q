...
def define_substitution(basename=None, cutsite=None, cutoff=None, primer_strand=None):
    ...
    for i in range(0, count):
        ...
        for j in range(0, len(md_string_list)):
            if md_string_list[j].isdigit():
                ...
                mismatch_base = data["Sequence"][i][
                    (clip_number + seq_length - 1) : (clip_number + seq_length)
                ]
                # print(bed_start,bed_end)
                if bed_start >= (int(cutsite) - int(cutoff)) and bed_end <= (
                    int(cutsite) + int(cutoff) - 1
                ):
                    define = "Y"
                    n = n + 1
                    # print([data.loc[[i]]])
    ...
    data_cutoff.to_csv(
        "unique/" + basename + "_Substitution_cutoff.tab",
        header=True,
        sep="\t",
        index=False,
    )
    ...
    germline_addup_st.to_csv(
        "unique/" + basename + "_Germline_addup_st.tab",
        header=True,
        sep="\t",
        index=False,
    )
    ...
    invertions.to_csv(
        "unique/" + basename + "_invertion.tab", header=True, sep="\t", index=False
    )
    ...
    interTransloc.to_csv(
        "unique/" + basename + "_interSV.tab", header=True, sep="\t", index=False
    )
    interTransloc_count = interTransloc["Qname"].count()

    germ_file = "unique/" + basename + "_Germline.tab"
    germ_addup_st = pd.read_csv(
        "unique/" + basename + "_Germline_addup_st.tab",
        sep="\t",
        index_col=False,
        low_memory=False,
    )
    germ_addup_indel = pd.read_csv(
        "unique/" + basename + "_Germline_addup_indel.tab",
        sep="\t",
        index_col=False,
        low_memory=False,
    )
    ...
    data.to_csv(
        "unique/" + basename + "_Germline_final.tab", header=True, sep="\t", index=False
    )
