...
def DSB_filter(basename, chromosome, cutsite, strand, de_method, yourfile):
    ...
    transloc["Seq_len"] = transloc["Sequence"].map(
        lambda x: 0 if x is np.nan else len(x)
    )
    ...
    for i in range(0, reads["Qname"].count()):
        ...
        count = dataset[
            (dataset["Rname"] == Rname)
            & (dataset["Junction"] >= start)
            & (dataset["Junction"] <= end)
        ]  # & (dataset['Strand'] == s)
    ...
    length = finalset["Barcode"].str.len()
    finalset["Length"] = length
    bc_char_cols = list(map(str, range(1, (bl + 1))))
    finalset = finalset.sort_values(by=["Rname", "Junction", "Length"], ascending=False)
    finalset = finalset.drop_duplicates(
        ["Rname", "Strand", "Prey_start", "Prey_end", *bc_char_cols], keep="first"
    )

    print(Full["Qname"].count())
    removed = Full[~Full["Qname"].isin(finalset["Qname"])]
    removed.to_csv(
        "results/" + basename + yourfile + "_dsbremove.tab",
        header=True,
        sep="\t",
        index=False,
    )
    ...
    # finalset.reset_index(level=finalset.index.names, inplace=True)
    finalset.to_csv(
        "results/" + basename + yourfile + "_dsb.tab",
        header=True,
        sep="\t",
        index=False,
    )
